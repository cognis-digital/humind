...
**humind** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `humind` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
humind ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
humind ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `humind` outputs:

```python
from cognis_connect import normalize, edgemesh
findings = [normalize(rec, source="humind") for rec in records]   # records = your JSON output
